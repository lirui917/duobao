...
class Rush extends Common
{
   ...
   */
   public function detail()
   {
        ...
        if($act_id == ""){
            $this->get_msg('103','夺购商品id为空');
        }
        //act_id base64解密
//        $act_ids=base64_decode($act_id);
        //商品属性
        $sql="SELECT act_name,goods_name,act_priod,act_price,act_join,act_sum,goods_id from ecs_goods_activity where act_id = $act_id ";
        $system=Db::query($sql); 
        if($system[0]['goods_id'] == ""){
            $this->get_msg('103','商品详情id为空');
        }
